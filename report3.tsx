...
export default function ReportScreen() {
  ...
  const [webviewKey, setWebviewKey] = useState(0); // To force reload WebView
  ...
  // When language changes, reload the WebView to update recognition language
  useEffect(() => {
    ...
    <body>
      <script>
        ...
        window.document.addEventListener('message', function(event) {
          var msg = event.data || event.message;
          if(msg === 'START') {
            ...
            recognition.onend = function() {
              isRecording = false;
              window.ReactNativeWebView.postMessage('STOPPED');
            };
            recognition.start();
            isRecording = true;
          }
          if(msg === 'STOP' && recognition && isRecording) {
            recognition.stop();
            isRecording = false;
          }
        });
  ...
  const handleWebViewMessage = (event: any) => {
    ...
    if (msg.startsWith('ERROR:')) {
      setIsRecording(false);
      Alert.alert('Voice Error', msg.replace('ERROR:', ''));
      return;
    }
    setFormData(prev => ({
      ...prev,
      description: prev.description ? prev.description + ' ' + msg : msg,
    }));
    setIsRecording(false);
  };
  ...
  const stopRecording = () => {
    ...
    // Force reload WebView to reset recognition instance
    setWebviewKey(prev => prev + 1);
  };
      ...
      <View style={styles.formCard}>
        ...
        <View style={styles.inputGroup}>
          ...
          <View style={styles.voiceRow}>
            ...
            <TouchableOpacity
              style={[styles.voiceButton, isRecording && { backgroundColor: '#e74c3c', borderColor: '#e74c3c' }]}
              onPress={isRecording ? stopRecording : startRecording}
            >
              <Ionicons name={isRecording ? 'mic-off' : 'mic'} size={28} color={isRecording ? 'white' : '#3498db'} />
              <Text style={{ color: isRecording ? 'white' : '#3498db', marginLeft: 6 }}>
...
const styles = StyleSheet.create({
  ...
  voiceButton: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#eaf6fb', borderRadius: 20, paddingHorizontal: 12, paddingVertical: 8, borderWidth: 1, borderColor: '#3498db' },
});
